...
## Tools used
  ...
  - [JWT tool](https://github.com/ticarpi/jwt_tool/tree/master) : jwt_tool.py is a toolkit for validating, forging, scanning and tampering JWTs (JSON Web Tokens).
...
<!-- ## Nmap scan on HOST port (32674)

```
0bytes> nmap -A -p 32674 188.166.150.33
...
After many attempts with the default usernames and passwords, I create a new account by clicking on the register button.
...
Thanks to the online tool [jwt io](https://jwt.io/), we can see what is in the token.

In the decode box we have the username and the public key. Save this public key value in a file.

After many research, I found this [tool](https://github.com/ticarpi/jwt_tool/tree/master). Then I try union injections to find the fulnerabilities to find the table who containt the flag. Below what I use to find this flag.

```bash
0bytes> ./jwt_tool.py [jwt token] -I -pc username -pv "admin' AND '1'='1' UNION SELECT 1,(SELECT top_secret_flaag FROM flag_storage),3;--" -X k -pk public.pem
```
...
And I use `burpsuite` to intercept the request and change the token
